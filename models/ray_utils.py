...
def get_ray_directions(H, W, focal, center=None):
    ...
    directions = torch.stack([(i.to(device) - cent[0]) / focal[0], (j.to(device) - cent[1]) / focal[1], torch.ones_like(i).to(device)], -1)  # (H, W, 3)
...
def get_rays(directions, c2w):
    ...
    device = torch.device('cuda')
    rays_d = directions.to(device) @ c2w[:3, :3].T.to(device)  # (H, W, 3)
    # rays_d = rays_d / torch.norm(rays_d, dim=-1, keepdim=True)
    # The origin of all rays is the camera origin in world coordinate
    rays_o = c2w[:3, 3].expand(rays_d.shape).to(device)   # (H, W, 3)
